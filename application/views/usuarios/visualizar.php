<?php echo form_open_multipart('', array('id'=> 'usuarios_add', 'autocomplete' => 'off')); ?>
	...
	<label style="display: block; float: left; width: 100%; padding: 0px;">
		...
		<select name="end_uf" style="width: 15%; min-width: 53px; margin-right: 6px;" required>
			...
			<?php foreach($ufs as $option): ?>
				<option value="<?php echo $option['id']; ?>" <?php echo (set_value('end_uf') == $option['id'] || $usuario['end_uf'] == $option['id']) ? 'selected="true"' : ''; ?>><?php echo $option['sigla']; ?></option>
			<?php endforeach; ?>
		</select>
		<select name="end_cidade" style="width: 50%; min-width: 136px;" required>
			<option>Cidade</option>
			<?php foreach($cidades as $option): ?>
				<option value="<?php echo $option['id']; ?>" <?php echo (set_value('end_cidade') == $option['id'] || $usuario['end_cidade'] == $option['id']) ? 'selected="true"' : ''; ?>><?php echo $option['nome']; ?></option>
			<?php endforeach; ?>
		</select>
...
<section class="boxform" style="float: left;">
	...
	<label>
		...
		<input type="text" name="remuneracao" placeholder="00,00" onkeypress="return(MascaraMoeda(this,'.',',',event))" value="<?php echo (set_value('remuneracao')) ? set_value('remuneracao') : $usuario['remuneracao']; ?>" required/>
